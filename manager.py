...
def manager():
    ...
    # load task file
    done = task2file.load(path_base + 'done.txt')
    todo = task2file.load(path_base + 'todo.txt')
    ...
    try:
        while len(todo) != 0:
            try:
                ...
                get_user_info.write2file(path_base + 'info.txt', data)
                print '[+] 已将用户信息保存至: info.txt'
                # add id_ to done
                done.add(id_)
                count += 1
                # to avoid task set expanding too fast, write them to file in time
                task2file.save(path_base + 'todo.txt', todo)
                task2file.save(path_base + 'done.txt', done)
            except Exception, e:
                print e
                exit()
    finally:
